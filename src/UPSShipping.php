...
//            $paymentNode = $shipmentNode->appendChild($xml->createElement('ItemizedPaymentInformation'));
        ...
        $ItemizedPaymentInformation = null;

        if ($shipment->getItemizedPaymentInformationTransportation()) {
            $ItemizedPaymentInformation = $xml->createElement('ItemizedPaymentInformation');
            $ItemizedPaymentInformation->appendChild($shipment->getItemizedPaymentInformationTransportation()->toNode($xml));
        }

        if ($shipment->getItemizedPaymentInformation()) {
            if (is_null($ItemizedPaymentInformation)) {
                $ItemizedPaymentInformation = $xml->createElement('ItemizedPaymentInformation');
            }
            $ItemizedPaymentInformation->appendChild($shipment->getItemizedPaymentInformation()->toNode($xml));
        }

        if (!is_null($ItemizedPaymentInformation)) {
            $shipmentNode->appendChild($ItemizedPaymentInformation);
        }
        ...
        if (isset($ratingMethodRequestIndicator)) {
            $shipmentNode->appendChild($xml->createElement('RatingMethodRequestIndicator'));
        }
